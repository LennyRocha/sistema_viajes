...
export class SalidasService {
    ...
    private requireCatalogoService() {
        ...
        if (!catalogoService) {
            this.logger.error('CATALOGO_SERVICE_URL no está configurada');
            throw new BadRequestException('Error de configuración del servidor');
        }
    ...
    private async getAutobusSalidaData(autobusId: number) {
        ...
        try {
            const response = await fetch(`${catalogoService}/autobuses/${autobusId}`);
    ...
    private getSalidaPrice(precios: unknown) {
        ...
        if (Array.isArray(priceObject.rutas)) {
            return priceObject.rutas.reduce(
                (total: number, ruta: { precio?: number | string }) =>
                ...
                ? {
                    ...
                    institucion: autobus.institucion
                        ? {
                            ...
                            nombre: autobus.institucion.nombre ?? null,
                            imagen_url: autobus.institucion.imagen_url ?? null,
                        }
                        : null,
                    servicios: (autobus.servicios ?? []).map((servicio) => ({
                        id: servicio?.servicio?.id ?? servicio?.id ?? null,
                        nombre: servicio?.servicio?.nombre ?? null,
                        descripcion: servicio?.servicio?.descripcion ?? null,
                        icono_nombre: servicio?.servicio?.icono_nombre ?? null,
                        activo: servicio?.activo ?? true,
                        config_servicio: servicio?.config_servicio ?? null,
                    })),
                }
                : null,
            viajeBase: viaje
    ...
    async create(dto: CreateSalidaDto) {
        ...
        if (!viaje) {
            ...
            throw new NotFoundException('El viaje base no existe');
        }
        ...
        try {
            [conductorResponse, autobusResponse] = await Promise.all([
                fetch(`${catalogoService}/conductores/salidas/${dto.conductorId}`),
                fetch(`${catalogoService}/autobuses/salidas/${dto.autobusId}`),
            ]);
        } catch (error) {
            this.logger.error(
                {
        ...
        await Promise.all([conductorResponse.json(), autobusResponse.json()]);
        ...
        try {
            ...
            this.logger.debug({ key: 'salidas:list' }, 'Caché eliminada');
        } catch (error) {
            this.logger.error(
                { err: error },
        ...
        this.logger.info({ salidaId: salida.id }, 'Salida creada correctamente');
    ...
    async findAll() {
        ...
        return Promise.all(salidas.map((salida) => this.mapSalida(salida)));
    }
